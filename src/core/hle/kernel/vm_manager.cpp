...
void VMManager::MergeAdjacentVMA(VirtualMemoryArea& left, const VirtualMemoryArea& right) {
    ...
    if (left.type == VMAType::AllocatedMemoryBlock &&
        ...
        if (left.offset == 0 && left.size == left.backing_block->size()) {
            ...
            memcpy(left.backing_block->data() + left.size, right.backing_block->data() + right.offset, right.size);
        } else {
            // Slow case: make a new memory block for left and right.
            auto new_memory = std::make_shared<PhysicalMemory>();
            new_memory->resize(left.size + right.size);
            memcpy(new_memory->data(), left.backing_block->data() + left.offset, left.size);
            memcpy(new_memory->data() + left.size, right.backing_block->data() + right.offset, right.size);
